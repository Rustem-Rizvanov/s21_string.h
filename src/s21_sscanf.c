...
// Функция считывает и переводит строку в long int в зависимости от системы
// счисления radix.
...
// Функция считывает и переводит строку в long double независимо от нотации, в
...
// Функция считывает из строки число типа int и сдвигает указатель на строку
// *str на необходимое количество символов вперед Возвращает 1, если было
// присвоено значение аргументу и 0 - если нет.
...
// Функция считывает из строки число типа double в любой нотации и сдвигает
// указатель на строку *str на необходимое количество символов вперед Возвращает
// 1, если было присвоено значение аргументу и 0 - если нет.
int getDouble(const char **str, va_list *argp, Holder **holderPointer,
              int mod) {
  int set = 0;
  char *fd = getFirstDigit(*str);
  if (*fd == '-' || *fd == '+') fd++;
...
// Функция записывает по адресу аргумента количество встреченных до этого
// символов в строке Возвращает 1, если было присвоено значение аргументу и 0 -
// если нет.
...
// Функция принимает в качестве аргумента адрес указателя на символ в форматной
// строке pointer to format line (PFL), считывает инструкцию и возвращает ее
// код. По окончании PFL указывает на следующий за прочтенной инструкцией
// символ. Когда встречается спецификатор (или модификатор), возвращает ноль.
...
// Функция преобразует число из строки в int, принимая адрес указателя на
// строку, начинающейся с первой цифры числа. Указатель по окончании работы
// функции будет указывать на первый символ после числа.
...
// Функция принимает в качестве аргумента адрес указателя на первый символ
// спецификатора pointer to format line (PFL), считывает спецификатор и
// возвращает его код. По окончании PFL указывает на следующий за спецификатором
// символ. В случае ошибки возвращает ноль.
